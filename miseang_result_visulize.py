...
"""
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        record = {}
        record["file_names"] = [os.path.join(image_root, vid_dict["file_names"][i]) for i in range(len(vid_dict["file_names"]))]
        record["height"] = vid_dict["height"]
        record["width"] = vid_dict["width"]
        record["length"] = len(vid_dict["file_names"])
            ...
            for anno in anno_dict_list:
                ...
                if isinstance(segm, dict):
                    if isinstance(segm["counts"], list):
                        ...
                        segm = segm # 이 부분에서 아래와 같이 "filter out invalid polygons"을 적용하고 싶기도 함 but 어떻게 ?
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", help="JSON file", default='../vis_test/hyeon/220918_r101_miseang/CDVA_MAX/results_vis_0.25.json')
    parser.add_argument("--output", help="output directory", default='../vis_test/hyeon/220918_r101_miseang/CDVA_MAX/output_visualize_0.25')
    parser.add_argument("--frames", help="frames directory", default='../vis_test/hyeon/220918_r101_miseang/CDVA_MAX/output_visualize')
