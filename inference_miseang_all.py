...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model_dir_path', type=str, default='./220918_r101_miseang/CDVA_MAX/',
    ...
    # dataset parameters
    parser.add_argument('--img_path', default='../miseang_vis_data/vis_format/validation/frame/')
    parser.add_argument('--ann_path', default='../miseang_vis_data/vis_format/validation/json/validation.json')
    #parser.add_argument('--img_path', default='../vis_test/hyeon/output_visualize/') ## vis aug img
    #parser.add_argument('--ann_path', default='../vis_test/hyeon/output_visualize/val.json')
    parser.add_argument('--save_path', default='../vis_test/hyeon/220918_r101_miseang/CDVA_MAX/')
    parser.add_argument('--dataset_file', default='Miseang')
...
CLASSES = ["janggeurae", "ohsangsik", "kimdongsik", "jangbaekki", "anyoungyi", "hanseokyul", "someone"]
...
def main(args):
    ...
    checkpoint_file_list = os.listdir(args.model_dir_path)
    checkpoint_file_list.sort()

    for e, checkpoint_file in enumerate(checkpoint_file_list):

#        if checkpoint_file != "checkpoint0014.pth":
#            continue
        if len(checkpoint_file) <= 14:
            continue

        model_path = args.model_dir_path + checkpoint_file
#        print('json  - epoch', e)
...
#                print("Process video: ", i)
                id_ = videos[i]['id']
                vid_len = len(videos[i]['file_names'])
                ...
                for idx in indices10:
                    ...
                    if queryid in hit_dict.keys():
                        hit_dict[queryid].append(idx % 8)
                    else:
                        hit_dict[queryid] = [idx % 8]
...
#                        print('skip')
        ...
        epoch = str(e).zfill(4)
        save_path = args.save_path + 'results_' + epoch + '.json'
#        save_path = args.save_path + 'results_0015.json'
        print(save_path)
