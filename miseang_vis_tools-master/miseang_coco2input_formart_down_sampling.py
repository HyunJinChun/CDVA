...
def coco_videos2vid(json_data, json_name, video_id):
    ...
    imageid_filename_dict = {}

    # sorted by image file name
    sorted_coco_image_list = sorted(coco_image_list, key=lambda d: d["file_name"])

    for idx, coco_image in enumerate(sorted_coco_image_list):
        if idx == 0:
        ...
        imageid_filename_dict[coco_image["id"]] = coco_image["file_name"]
    ...
    return vid_video_format, imageid_filename_dict
...
def coco_annotations2vid(json_data, start_instance_id, video_id, height, width, length, id_list, imageid_filename_dict):
...
'''
CVAT가 image_id를 역순으로 부여하는 문제
'''
def convert_annotations(annotations_list, start_instance_id, video_id, height, width, length, id_list, imageid_filename_dict):
    # track_dict['track_id']['image_id'] = anno
    track_dict = {}

    # sort annotation_list by file_name
    for anno in annotations_list:
        anno["file_name"] = imageid_filename_dict[anno["image_id"]]

    annotations_list = sorted(annotations_list, key=lambda d: d["file_name"])

    real_imageid_filename_list = list(imageid_filename_dict.values())
    real_imageid_filename_dict = {}

    for idx, filename in enumerate(sorted(real_imageid_filename_list)):
        real_imageid_filename_dict[filename] = idx + 1

    # sort
    # image_id -> file_name 순서
    for anno in annotations_list:
        ...
        anno["image_id"] = real_imageid_filename_dict[file_name]

        if track_id not in track_dict.keys():
            track_dict[track_id] = {}

        track_dict[track_id][real_imageid_filename_dict[file_name]] = anno
...
def down_sampling(vis_json):
    json_per_class = {}

    for anno in vis_json['annotations']:
        category_id = anno['category_id']

        if category_id not in json_per_class:
            json_per_class[category_id] = []

        json_per_class[category_id].append(anno)

    random.shuffle(json_per_class[1])
    random.shuffle(json_per_class[7])

    json_per_class[1] = json_per_class[1][:16]
    json_per_class[7] = json_per_class[7][:23]

    new_anno_list = []
    new_id = 1

    for class_id in json_per_class.keys():
        for anno in json_per_class[class_id]:
            anno['id'] = new_id
            new_id += 1
            new_anno_list.append(anno)

    vis_json['annotations'] = new_anno_list

    return vis_json


if __name__ == '__main__':
    coco_root = '../miseang_vis_data/coco_format/train/json'
    # vis_root = '../miseang_vis_data/vis_format_down_sample/train/json'
    vis_root = '../miseang_vis_data/vis_format_down_sample'
    name = 'train.json'

    '''
    coco_root = '../miseang_vis_data/coco_format/validation/json'
    # vis_root = '../miseang_vis_data/vis_format_down_sample/validation/json'
    vis_root = '../miseang_vis_data/vis_format_down_sample'
    name = 'validation.json'
    '''
    '''
    coco_root = '../miseang_vis_data/coco_format/test/json'
    vis_root = '../miseang_vis_data/vis_format_down_sample/test/json'
    name = 'test.json'
    '''
    ...
    for idx, coco_file in enumerate(coco_file_list):
        ...
        vid_video_forma, imageid_filename_dict = coco_videos2vid(coco_json, coco_file, video_id)
        # total_imageid_filename_dict[video_id] = imageid_filename_dict

        vid_json['videos'].append(vid_video_forma)
    ...
    # vid_json sampling
    # "janggeurae" : 1, "someone" : 7

    vid_json = down_sampling(vid_json)
