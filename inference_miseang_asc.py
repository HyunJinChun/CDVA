...
inference_miseang_asc.py : score를 오름차순으로 정렬
inference_miseang.py : score 내림차순
'''
import argparse
import datetime
import json
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model_path', type=str, default='./r101_miseang_face/checkpoint0002.pth',
    ...
    # dataset parameters
    parser.add_argument('--img_path', default='../miseang_vis_dataset/val/frame/')
    parser.add_argument('--ann_path', default='../miseang_vis_dataset/annotations/instances_val_sub.json')
    parser.add_argument('--save_path', default='../vis_test/hyeon/copy_paste/mask_tracklet_220503/results.json')
...
def main(args):
        ...
        for i in range(vis_num):
            ...
            for inst_id in hit_dict.keys():
                ...
                for class_id in hit_dict[inst_id]:
                    ...
                    instance = {'video_id': id_, 'video_name': file_names[0][:video_name_len], 'score': float(score),
    ...
    result.sort(key=lambda key: key['score']) # ascending order
    result.sort(key=lambda key: key['video_id'])
