...
def apply_copy_paste(image_list, target, pasted_image_list, pasted_target, now_frames, target_key):
    ...
    # Paste 대상이 되는 객체 선택
    choice_target_list_dict = select_pasted_instance(pasted_target_list_dict, target_key)
    ...
    for frame_num in range(now_frames):
        ...
        # mask_list = choice_target_list_dict['masks'][:frame_num]
        ...
        for mask in mask_list[1:]:
            alpha += mask > 0
            # alpha = np.logical_or(alpha, mask)
        ...
        for m in mask_list[1:]:
            # alpha = torch.logical_or(alpha, m)
        ...
        '''
        masks = target_list_dict['masks'][:frame_num]
        paste_masks = choice_target_list_dict['masks'][:frame_num]
        '''
        ...
        '''
        boxes = target_list_dict['boxes'][:frame_num]
        paste_boxes = choice_target_list_dict['boxes'][:frame_num]
        '''
        boxes = []
    ...
    '''
    cp_boxes_list = torch.tensor(cp_boxes_list, dtype=torch.float32).transpose(1, 0)
    cp_mask_list = torch.tensor(np.stack(cp_mask_list, axis=0), dtype=torch.bool).transpose(1, 0)
    '''
    tmp_cp_boxes_list = []
    tmp_cp_masks_list = []
    ...
    '''
    for frame_num in range(now_frames):
        tmp_cp_boxes = torch.tensor(cp_boxes_list[frame_num], dtype=torch.float32)
        tmp_cp_boxes_list.append(tmp_cp_boxes)

        tmp_cp_masks = torch.tensor(cp_mask_list[frame_num], dtype=torch.bool)
        tmp_cp_masks_list.append(tmp_cp_masks)

    tmp_cp_masks_list = torch.stack(tmp_cp_masks_list, dim=0)
    tmp_cp_masks_list = tmp_cp_masks_list.view(-1, tmp_cp_masks_list.shape[-2], tmp_cp_masks_list.shape[-1])
    '''
    ...
    for key in target_key:
        ...
        for i in range(5):
            ...
            for j in range(instance_cnt):
                target_list_dict[key][i].append(target[key][i+5*j])

            for j in range(pasted_instance_cnt):
                pasted_target_list_dict[key][i].append(pasted_target[key][i + 5 * j])
...
def select_pasted_instance(pasted_target_list_dict, target_key):
    '''
    ...
    if instance_cnt > 0:
        random_choice = [random.randint(0, instance_cnt - 1)]
    else:
        random_choice = None
