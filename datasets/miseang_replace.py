...
import time
import numpy as np
from datasets import transforms_copy_paste_replace
...
class MiseangDataset:
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            if self._transforms is not None and self.copy_paste_transforms is not None: # 100% 확률
                ...
                background_img, background_target, target = self.get_background_target(background_idx, target, instance_choice, self.category) # 빈 공간을 고려하지 않고 background clip 인물의 크기 정보만을 이용한 get_background_target
    ...
    def get_background_target(self, idx, target_list_dict, instance_choice, category):
        instance_check = False
        while not instance_check:
            vid, frame_id = self.img_ids[idx]  ## ex) vid:0, frame_id:32
            vid_id = self.vid_infos[vid]['id']
            ...
            if category is False: ## a.k.a ObjectMix (모든 인물, 원래 자리에)
...
def make_copy_paste_transforms(size, position, GD, PD, all_frames):
    cp_normalize = T.Compose([
        transforms_copy_paste_replace.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    ...
    resize_transforms = transforms_copy_paste_replace.RandomResize(max_size=768)
    check = T.Check()
    hsv_transforms = transforms_copy_paste_replace.PhotometricDistort()
    cp_transforms = transforms_copy_paste_replace.CopyAndPaste()
    cp_compose = transforms_copy_paste_replace.CopyPasteCompose(flip_transforms, resize_transforms, check, hsv_transforms,
                                                                      cp_transforms, cp_normalize, normalize, scales, size, position, GD, PD, all_frames)
...
def build(image_set, args):
    ...
    if args.dataset_file == 'Miseang':
        ...
        print('use Miseang dataset - miseang_replace.py')

        if args.replace is True and image_set == 'train':
            copy_paste_tfs = make_copy_paste_transforms(size=args.size, position=args.position, GD=args.GD, PD=args.PD, all_frames=args.all_frames)
            print('make copy paste transforms - replace')
        else:
            copy_paste_tfs = None
