...
class CopyPasteCompose(object):
    def __init__(self, flip_transforms, resize_transforms, check, hsv_transforms, cp_transforms, cp_normalize, normalize, scales, size, position, GD, PD, all_frames, foreground):
        ...
        self.foreground = foreground # MVDA - calculate foreground/background humans
    ...
    def __call__(self, image, target, pasted_image, pasted_target, now_frames):        #
        ...
        # copy paste
        image, target = self.cp_transforms(image, target, pasted_image, pasted_target, now_frames, self.size, self.position, self.all_frames, self.foreground)
...
class CopyAndPaste(object):
    ...
    def __call__(self, image, target, pasted_image, pasted_target, now_frames, size, position, all_frames, foreground):
        cp_image_list, new_target = apply_mask_copy_paste(image, target, pasted_image, pasted_target, now_frames, self.target_key, size, position, all_frames, foreground) ##
...
# ============== 수정한 메소드 =============== #
def apply_mask_copy_paste(image_list, target, pasted_image_list, pasted_target, now_frames, target_key, size, position, all_frames, foreground):
    cp_image_list = []
    cp_mask_list = []
    cp_boxes_list = []
...
def mask_tracklet_usebbox_multi(image_size, target_list_dict, choice_target_list_dict, pasted_image_list, now_frames, blank_space, size_, position_):
    ...
    # # print('resize 이전 크기 :', t_size) #####
    # background_size = []
    # for i in range(len(target_list_dict['labels'])):
    #     b_size = extract_cuboid_region(target_list_dict['boxes'], image_size, now_frames, idx=i)
    #     if b_size is None or abs(b_size[3] / image_size[1]) > 0.7:
    #         continue
    #     else:
    #         background_size.append(b_size[3])
    #
    # if len(background_size) == 0:
    #     return None, None
    #
    # tmp = False
    # for i in background_size:
    #     resize_ratio = i / t_size[0] # background_width / target_width
    #     size = math.floor(t_size[0] * resize_ratio), math.floor(t_size[1] * resize_ratio) # (w,h) tuple
    #     if size[0] > image_size[1] or size[1] > image_size[0]:
    #         size = t_size
    ...
    #     crop_image_width, crop_image_height = pasted_image_list[0].size
        ...
        # 3. 크기 비교 후 적당한 게 있을 때, 선택된 인물이 빈 공간보다 과도하게 더 크다면 return None
        # 비율 계산 후 120% 이상인 경우는 return choice_target_list_dict

        # box width 정보를 miseang_using_getitem.py에서 했는데 여기서 한 번 더 하는 이유 : resize, flip에 의해 변경되었을 가능성이 있음
...
def cal_all_blank_space(size, target, fore):
    ...
    for box in target['boxes']:
        ...
        if fore is True:
            # ******** 인물이 일정 크기 이상인 경우에만 inst_box_list에 box 정보를 유지하도록 코드 작성하기
            print()
    ...
    region_list.append(min(left_region_list))
    region_list.append(min(right_region_list))
