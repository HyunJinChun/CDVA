...
from datasets import transforms_ObjectMix_MAX
...
class MiseangDataset:
    ...
    def __getitem__(self, idx):
        ...
        augClip_img, augClip_target = self.copy_paste_transforms(background_img, background_ann, target_img,
                                                                 ...
                                                                 self.targetClass)  # VideoMix CopyPaste
...
def make_copy_paste_transforms():
    cp_normalize = T.Compose([
        transforms_ObjectMix_MAX.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    ...
    resize_transforms = transforms_ObjectMix_MAX.RandomResize(max_size=768)
    check = T.Check()
    cp_transforms = transforms_ObjectMix_MAX.CopyAndPaste()
    cp_compose = transforms_ObjectMix_MAX.CopyPasteCompose(flip_transforms, resize_transforms, check, cp_transforms,
...
def build(image_set, args):
    ...
    if args.dataset_file == 'Miseang':
        ...
        print('use Miseang dataset - miseang_ObjectMix_MAX.py')

        print('ObjectMix :', args.ObjectMix)

        if args.ObjectMix is True and image_set == 'train':
            copy_paste_tfs = make_copy_paste_transforms()
            print('make ObjectMix transforms')
        else:
            copy_paste_tfs = None
