...
class MiseangDataset:
    def __init__(self, img_folder, ann_file, transforms, no_transforms, return_masks, num_frames, copy_paste_transforms):
        ...
        self.none_vid_list, self.none_vid_len, self.only_none_vid = self.get_clips_without_targetClasses(self.add_frame)  # 부족 인물 클래스들에 속한 어떤 인물도 포함되어 있지 않은 클립들
    ...
    def __getitem__(self, idx):
            ...
            if vid not in self.only_none_vid or (sum(self.requiredClips) ==0 and sum(self.add_clips) == 9000): # 배경에 targetClass 인물이 있거나 보강이 완료되면 CP X
...
def build(image_set, args):
    ...
    if args.dataset_file == 'Miseang':
        ...
        print('use Miseang dataset - miseang_VideoMix_MAX2.py')
