...
def apply_mask_copy_paste(image_list, target, pasted_image_list, pasted_target, now_frames, target_key, size, position, all_frames):
    ...
    if size is True or position is True:
        if all_frames is True:
            blank_space = cal_all_blank_space(target['size'], target_list_dict)
        else:
            blank_space = cal_blank_space(target['size'], target_list_dict) ################### 220818
...
def mask_tracklet_usebbox_multi(image_size, target_list_dict, choice_target_list_dict, pasted_image_list, now_frames, blank_space, size, position):
...
def cal_all_blank_space(size, target):
    h, w = size
    blank_space = []  # left : ['left', left_region], right : ['right', right_region], max_gap : ['max_gap', max_gap_region, max_gap(list)]
    left_region_list = []
    right_region_list = []
