...
def apply_copy_paste(image_list, target, pasted_image_list, pasted_target, now_frames, target_key, targetClass):
    ...
    for frame_num in range(now_frames):
        ...
        for n in range(now_frames):
            for i in range(len(pasted_target_list_dict['masks'][n])):
                mask_list.append(pasted_target_list_dict['masks'][n][i])
        ...
        paste_masks = []
        for i in range(len(pasted_target_list_dict['masks'][frame_num])):
            paste_masks.append(pasted_target_list_dict['masks'][frame_num][i].numpy())
        ...
        paste_boxes = []
        for i in range(len(pasted_target_list_dict['boxes'][frame_num])):
            paste_boxes.append(pasted_target_list_dict['boxes'][frame_num][i].numpy())

        cp_boxes = bboxes_copy_paste(boxes, paste_boxes, masks, paste_masks, alpha)
        cp_boxes_list.append(cp_boxes)

    # label extend
    cp_labels = torch.cat([target_list_dict['labels'], pasted_target_list_dict['labels']], dim=0)
