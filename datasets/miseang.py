...
from datasets import transforms_simple_copy_paste
...
def make_copy_paste_transforms():
    cp_normalize = T.Compose([
        transforms_simple_copy_paste.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    ...
    resize_transforms = transforms_simple_copy_paste.RandomResize(max_size=768)
    check = T.Check()
    cp_transforms = transforms_simple_copy_paste.CopyAndPaste()
    cp_compose = transforms_simple_copy_paste.CopyPasteCompose(flip_transforms, resize_transforms, check, cp_transforms,
...
def build(image_set, args):
    ...
    if args.dataset_file == 'Miseang':
        ...
        print('use Miseang dataset - miseang.py')

        status = None

        print('simple_cp :', args.simple_cp, 'simple_cp_one :', args.simple_cp_one)
