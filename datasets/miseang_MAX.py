...
from datasets import transforms_B_AUG_MAX

class MiseangDataset:
    def __init__(self, img_folder, ann_file, transforms, no_transforms, return_masks, num_frames, copy_paste_transforms, status):
    ...
    def __getitem__(self, idx):
        ...
        if idx >= len(self.img_ids) and sum(self.requiredClips) == 0:
            f = False
        ...
        augClip_img, augClip_target = self.copy_paste_transforms(background_img, background_ann, target_img, target_ann, self.num_frames, self.targetClass)  # B-Aug CopyPaste
...
def make_miseang_transforms(image_set, vt):
    normalize = T.Compose([
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
...
def make_copy_paste_transforms():
    ...
    resize_transforms = transforms_B_AUG_MAX.RandomResize(max_size=768)
    check = T.Check()
    cp_transforms = transforms_B_AUG_MAX.CopyAndPaste()
    cp_compose = transforms_B_AUG_MAX.CopyPasteCompose(flip_transforms, resize_transforms, check, cp_transforms,
...
def build(image_set, args):
    ...
    if args.dataset_file == 'Miseang':
        ...
        print('use Miseang dataset - miseang_B_AUG_MAX.py')

        status = None

        print('simple_cp :', args.simple_cp)

        if (args.simple_cp is True or args.simple_cp_one is True) and image_set == 'train':
            copy_paste_tfs = make_copy_paste_transforms()
            vt = True
            print('make copy paste transforms')
            if args.simple_cp_one is True:
                status = 'simple_cp_one'
                print('simple_cp_one')
        else:
            print('no copy paste transforms')
            copy_paste_tfs = None
            if args.video_transform is True:
                vt = True
            else:
                vt = False

        dataset = MiseangDataset(img_folder, ann_file, transforms=make_miseang_transforms(image_set, vt), no_transforms=no_transforms(), return_masks=args.masks,
